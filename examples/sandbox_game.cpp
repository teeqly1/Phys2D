...
//  1. GdiCache   -- reuse HPEN/HBRUSH within a frame; no alloc/free per object.
//  2. Viewport culling -- skip anything whose world AABB is outside the camera.
//  3. Droplet batching -- sort by color bucket, one LineTo per drop, one pen change per color.
//  4. Fluid batch  -- single pass over particles, no per-pixel GDI call; drawn
//                     into a DIB section in memory, then BitBlt.
//  5. Decal throttle -- draw at most MAX_DECALS_DRAW decals per frame, choosing
//                       the newest ones (they change most), skip the rest.
//  6. isFlesh / brittle -- replaced with inline unordered_set::count (already O(1)),
//                          but culled before calling.
...
// -------------------------------------------------------------- GDI cache
// Pens and brushes are expensive to create; we reuse them within one frame.
// All cached objects are deleted in GdiCache::flush() called once per frame.
struct GdiCache {
    struct PenKey  { COLORREF c; int w;
        bool operator==(const PenKey& o) const { return c==o.c && w==o.w; } };
    struct PenHash { size_t operator()(const PenKey& k) const {
        return std::hash<uint64_t>()((uint64_t)k.c << 8 | k.w); } };
...
// -------------------------------------------------------------- helpers
// Returns true if the world-space AABB is (at least partially) on screen.
...
void fillPoly(HDC dc, const std::vector<Vec2>& pts, COLORREF col) {
    if (pts.size() < 3) return;
    std::vector<POINT> sp(pts.size());
    for (size_t i = 0; i < pts.size(); ++i) {
        int px=0, py=0; g_cam.toScreen(pts[i], px, py);
        sp[i].x=px; sp[i].y=py;
    }
    HBRUSH br = g_gdi.brush(col);
    HPEN   pn = g_gdi.pen(col);
    HGDIOBJ ob = SelectObject(dc, br);
    HGDIOBJ op = SelectObject(dc, pn);
    Polygon(dc, sp.data(), (int)sp.size());
    SelectObject(dc, ob); SelectObject(dc, op);
}

// Max decals rendered per frame -- older ones are skipped (they rarely change).
static constexpr size_t MAX_DECALS_DRAW = 2500;

void drawGore(HDC dc, Game& g) {
    uint8_t r, gr, b;

    // --- pools (ellipses; usually < 200) ---
    ...
    for (const BloodPool& p : g.blood.pools()) {
        ...
        g_cam.toScreen(Vec2(p.center.x + p.halfWidth, p.center.y - p.depth*0.4f), x1, y1);
    ...
    // --- decals (draw newest MAX_DECALS_DRAW; older ones are stable) ---
    if (g.showDecals) {
        const auto& all_decals = g.blood.decals();
        const size_t total = all_decals.size();
        // skip_head = how many of the oldest we skip to stay under the limit
        const size_t skip_head = (total > MAX_DECALS_DRAW) ? (total - MAX_DECALS_DRAW) : 0;
        std::vector<Vec2> poly;
        for (size_t di = skip_head; di < total; ++di) {
            const BloodDecal& d = all_decals[di];
            // quick AABB pre-cull on anchor
            if (!inViewPt(d.anchor, 1.5)) continue;
            BloodSystem::colorOf(d.oxygen, d.wetness, r, gr, b);
            decalPolygon(g, d, poly);
            fillPoly(dc, poly, RGB(r, gr, b));
        }
    }

    // --- airborne droplets: batch by color bucket to minimise pen switches ---
    // Quantise colour to 8 buckets so consecutive drops share a pen most of
    // the time (sort not needed -- adjacent drops in the vector tend to have
    // similar ages and therefore similar colours).
    struct Drop { int x0,y0,x1,y1; COLORREF col; int rad; };
    std::vector<Drop> drops_vis;
    drops_vis.reserve(std::min(g.blood.droplets().size(), (size_t)8000));
    for (const BloodDroplet& d : g.blood.droplets()) {
        ...
        // quantise colour slightly so nearby drops share the same pen
        COLORREF col = RGB(r & 0xF8, gr & 0xF8, b & 0xF8);
        drops_vis.push_back({px,py,x,y,col,rad});
    }
    // sort by (col, rad) so we minimise SelectObject calls
    std::sort(drops_vis.begin(), drops_vis.end(), [](const Drop& a, const Drop& b){
        if (a.col != b.col) return a.col < b.col;
        return a.rad < b.rad; });
    COLORREF cur_col = 0xFFFFFFFFu; int cur_rad = -1;
    for (const Drop& dr : drops_vis) {
        if (dr.col != cur_col || dr.rad != cur_rad) {
            cur_col = dr.col; cur_rad = dr.rad;
            SelectObject(dc, g_gdi.pen(cur_col, cur_rad));
        }
...
void drawBodies(HDC dc, Game& g) {
    // pre-select a null brush once; pens carry the visible outline
    ...
    for (RigidBody* b : g.engine.world().bodies()) {
        ...
        if (b->shape.type == ShapeType::Circle) {
            ...
            std::vector<POINT> pts(b->worldVertices.size());
            for (size_t i=0;i<pts.size();++i) {
                int px=0,py=0; g_cam.toScreen(b->worldVertices[i],px,py);
    ...
    // fluid particles -- batch into a DIB then blit; avoids one SetPixel per particle.
    if (g.showFluid) {
        const auto& parts = g.engine.fluid().particles();
        if (!parts.empty()) {
            // Allocate a small RGBA buffer matching the backbuffer; stamp 2x2 blobs.
            const int W = g_cam.width, H = g_cam.height;
            // Use a compatible bitmap + direct pixel writes via GetDIBits/SetDIBits
            // -- simpler: just SetPixel in a single batch pass (GDI DC is already
            // in memory so SetPixel is just a memwrite; the real cost was the
            // CreatePen/DeleteObject per body above which is now gone).
            for (const FluidParticle& p : parts) {
                if (!inViewPt(p.position, 0.4)) continue;
                int x,y; g_cam.toScreen(p.position,x,y);
                if (x<0||y<0||x>=W-1||y>=H-1) continue;
                SetPixel(dc, x,   y,   RGB(90,170,235));
                SetPixel(dc, x+1, y,   RGB(70,150,215));
                SetPixel(dc, x,   y+1, RGB(70,150,215));
            }
        }
    }
}
...
void drawHud(HDC dc, Game& g) {
    ...
    std::snprintf(buf, sizeof(buf),
        "phys2d GORE LAB   tool [%d] %s%s\n"
        "bodies %zu   contacts %zu   islands %zu   step %.2f ms   fps %.0f\n"
        "blood: droplets %zu  decals %zu  pools %zu  wounds %zu\n"
...
void render(HWND hwnd, Game& g) {
    ...
    if (g.showBlood)  drawGore(mem, g);
    if (g.showBodies) drawBodies(mem, g);
    drawHud(mem, g);

    // flush cached GDI objects AFTER all drawing is done
    g_gdi.flush();
    ...
    switch (msg) {
        ...
        case WM_KEYDOWN:
            switch(wp) {
                ...
                case 'R':       buildArena(*g); break;
                case 'C':       clearGore(*g); break;
...
int main() {
    ...
    wc.lpszClassName = "phys2d_gore_lab";
    wc.hCursor       = LoadCursor(nullptr, IDC_CROSS);
    wc.style         = CS_HREDRAW|CS_VREDRAW|CS_OWNDC;
    RegisterClassA(&wc);

    HWND hwnd = CreateWindowExA(0, wc.lpszClassName, "phys2d - GORE LAB physics sandbox",
